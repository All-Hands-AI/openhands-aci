...
class OHEditor:
    ...
    # Fenced diff is adapted from the logic in Aider / OpenHands runtime/utils/edit.py
    # Copyright Paul Gauthier / All-Hands-AI
    # Original licensed under Apache 2.0: http://www.apache.org/licenses/LICENSE-2.0
    def fenced_replace(self, path: Path, search: str, replace: str) -> CLIResult:
        """
        Implement Fenced Diff (SEARCH/REPLACE) edit logic.
        """
        # Use 'view' command context for path validation, but check file existence specifically
        self.validate_path('view', path) # Check if path is absolute etc.
        if not path.exists():
             # Handle file creation if search is empty
            if not search.strip():
                self.write_file(path, replace)
                # Add history for creation? ACI doesn't seem to for 'create', maybe skip?
                # self._history_manager.add_history(path, '') # Add empty history?
                return CLIResult(
                    path=str(path),
                    new_content=replace,
                    prev_exist=False,
        ...
        # Use original_content read by read_file which handles encoding
        original_lines = original_content.splitlines(keepends=True)
        search_lines = search.splitlines(keepends=True)
        replace_lines = replace.splitlines(keepends=True)

        # Ensure blocks aren't empty lists if input was just whitespace
