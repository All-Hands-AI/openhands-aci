...
if INDEXING_AVAILABLE:
    ...
    # Placeholder implementations when libcst is not available
    class CompressTransformer:
        def __init__(self, keep_constant=True):
            raise ImportError(
                f"Indexing dependencies not available: {_missing_import}. "
                "Install with: pip install openhands-aci[indexing]"
            )
